...
static int parse_hex_digit(unsigned *b, const char **p) {
    if(**p >= '0' && **p <= '9') {
        *b = static_cast<unsigned>(**p) - '0';
        (*p)++;
        return 1;
    }

    if(**p >= 'a' && **p <= 'f') {
        *b = static_cast<unsigned>(**p) - 'a' + 10;
        (*p)++;
        return 1;
    }
...
static void append_byte(struct byte_vector *v, unsigned b) {
    ...
    v->bytes[v->size++] = static_cast<unsigned char>(b);
}
...
static void test_encoding(const struct byte_vector *input,
    ...
    escapeless::encode(takeouts_map.bytes, takeouts->bytes, takeouts->size,
...
static void test_decoding(const struct byte_vector *input,
    ...
    escapeless::decode(takeouts_map.bytes, takeouts->bytes, takeouts->size,
                       block.bytes, block.size);
